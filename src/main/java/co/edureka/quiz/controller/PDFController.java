...
public class PDFController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			writer.setEncryption(username.getBytes(), username.getBytes(), PdfWriter.ALLOW_PRINTING,
		...
		try {
			for (QuizQuestion qe : reviewQuestionList) {
				...
				document.add(new Phrase("Q" + qi + ". " + que,
						new Font(FontFamily.HELVETICA, 14, Font.BOLD, BaseColor.BLACK)));
				qi++;
				document.add(new Paragraph("\n"));
				int i = 1;
				...
				document.add(new Phrase("Correct Answer : " + correct,
						new Font(FontFamily.HELVETICA, 12, Font.NORMAL, BaseColor.GREEN)));

				document.add(new Paragraph("\n"));
				if (qe.getUserSelected() == -1) {
					document.add(
							new Phrase("Unanswered", new Font(FontFamily.HELVETICA, 12, Font.NORMAL, BaseColor.RED)));
				} else {
					document.add(new Phrase("You Chose : " + qe.getUserSelected(),
							new Font(FontFamily.HELVETICA, 12, Font.NORMAL, new BaseColor(9, 154, 251))));
				}
				document.add(new Paragraph("\n"));
		...
		try {
			document.add(new Phrase("This PDF is created using iText, Thanks to Bruno Lowagie",
					new Font(FontFamily.COURIER, 16, Font.BOLDITALIC, BaseColor.BLACK)));
